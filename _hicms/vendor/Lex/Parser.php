...
class Lex_Parser
{
	...
	public function parse($text, $data = array(), $callback = false, $allow_php = false)
	{
		...
		if (strstr($text, "{{"))  {
			$text = $this->parse_callback_tags($text, $data, null);
		}
	...
	public function parse_callback_tags($text, $data, $callback)
	{
		...
		while (preg_match($regex, $text, $match, PREG_OFFSET_CAPTURE))
		{
			...
			if (isset($match[2]))
			{
				...
				foreach ($parameters as $key => $value) {
					if (preg_match('/\{\s*('.$this->variable_regex.')\s*\}/', $value)) {
						$value = str_replace('{', '{{', $value);
						$value = str_replace('}', '}}', $value);
						$parameters[$key] = $this->parse_variables($value, $data);
					}
				}
			}
			$content = '';
			...
			if ($name != 'content' && $replacement == '') {
				...
				if (isset($data[$name])) {
					...
					if (isset($parameters['sort_by'])) {
						...
						// Add 'first' and 'last' variables
						$i = 1;
						$total_results = count($values);
						foreach($values as $key => $value)
						{
							$values[$key]['first'] = $i == 1 ? true : false;
              $values[$key]['last'] = $i == $total_results ? true : false;
              $i++;
						}
					...
					if (is_array($sdata)) {
						if (isset($sdata[$name]) && $sdata[$name] <> '') {
							$replacement = $this->parse_variables("{{ $name }}$content{{ /$name }}", $sdata, $callback);
						} else {
							$replacement = "";
						}
					}
					...
					// $$$ MUBS
					if ($callback == null) {
						$text = $this->create_extraction('__variables_not_callbacks', $text, $text, $text);
					}
				}
			}
		...
		// reinject?
		if ($callback == null) {
			$text = $this->inject_extractions($text, '__variables_not_callbacks');
		}
	...
	protected function process_condition_var($match)
	{
		...
		{
			// ### STATAMIC: If variable does not exist was throwing an error
		...
		if ($modifiers) {
			foreach ($modifiers as $mod) {
				...
				if ($modifier_name == 'empty') {
					$data = HiCMS_helper::array_empty($data);
				}

				if ($modifier_name == 'not_empty') {
					$data = HiCMS_helper::array_empty($data) === false;
				}

				if ($modifier_name == 'trim') {
					$data = trim($data);

				} else if ($modifier_name == 'upper') {
        	...
        	$data = HiCMS_helper::slugify($data);

        } else if ($modifier_name == 'deslugify') {
        	$data = HiCMS_helper::deslugify($data);
        	...
        	$data = strip_tags($data);
      		...
      		if (isset($modifier_params[0])) {
      			$remainder = $data % $modifier_params[0];
						$data = $remainder;
					}
    			...
    			$space = strrpos($data, ' ');
    			if ($space !== false) {
        		$data = substr($data, 0, $space).'&nbsp;'.substr($data, $space + 1);
        	}
	...
	protected function parse_php($text)
	{
		...
		try {
			...
			# Ignore variable not define exception
		}  
